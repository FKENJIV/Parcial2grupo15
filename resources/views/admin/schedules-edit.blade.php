<x-layouts.app :title="__('Editar Horario')">
    <!-- Header -->
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-3xl font-bold text-gray-800">Editar Horario del Grupo</h2>
                <p class="text-gray-600 mt-1">Docente: {{ $group->teacher->name }}</p>
            </div>
            <a href="{{ route('admin.schedules.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
    ...
    <!-- Form -->
    <form action="{{ route('admin.schedules.update', $group->id) }}" method="POST" class="space-y-6" id="groupForm">
        @csrf
        @method('PUT')
        ...
        <section class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="subject" required value="{{ old('subject', $group->subject) }}" class="w-full rounded-lg border border-gray-200 px-4 py-3 bg-white text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-2">Código del Grupo *</label>
                    <input type="text" name="code" required value="{{ old('code', $group->code) }}" class="w-full rounded-lg border border-gray-200 px-4 py-3 bg-white text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-2">Cupo Máximo de Alumnos *</label>
                    <input type="number" name="max_students" required min="1" max="100" value="{{ old('max_students', $group->max_students) }}" class="w-full rounded-lg border border-gray-200 px-4 py-3 bg-white text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-2">Aula / Ambiente *</label>
                    <input type="text" name="classroom" required value="{{ old('classroom', $group->classroom) }}" class="w-full rounded-lg border border-gray-200 px-4 py-3 bg-white text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
            </div>
        </section>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @php
                            ...
                            foreach($group->schedules as $schedule) {
                                $existingSchedules[$schedule->day . '_' . $schedule->time_block] = true;
                            }
                        @endphp
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            @php
                                $timeBlock = 7 + $index;
                                $key = $dayEn . '_' . $timeBlock;
                                $isChecked = isset($existingSchedules[$key]);
                            @endphp
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('admin.schedules.index') }}" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors">
                Cancelar
            </a>
        </div>
    </form>
